...
import { OnDiskFileManager } from './fm/on-disk-file-manager';
...
export class NoirWasmContractCompiler {
  ...
  public async compile(): Promise<any> {
    ...
    let fileManager: InMemoryFileManager | OnDiskFileManager;
    ...
    if (typeof Buffer === 'undefined') {
      ...
      fileManager = new OnDiskFileManager(join(cacheRoot, 'noir_wasm'));
    }

    const noirPackage = NoirPackage.new(this.#projectPath, fileManager);

    this.#log(`Compiling circuit at ${noirPackage.getEntryPointPath()}`);
    ...
    initializeResolver((sourceId: any) => {
      console.log("finding file", sourceId)
      try {
    ...
    try {
      ...
      if (typeof Buffer === 'undefined') await Promise.resolve(initWasm());
      const program = await compile(noirPackage.getEntryPointPath(), false, dependencyManager.getPackageNames());
